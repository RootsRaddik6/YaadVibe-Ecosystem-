...
import { getTransportForParish, getTownsForParish, findTownBySlug } from "@/lib/parish";
import { buildMetaForTransport } from "@/lib/metadata";
import { requireApiKey, rateLimit } from "@/lib/auth";

/**
 * GET /api/parishes/[parish]/towns/[town]/transport
 * Query params:
 *  - page, limit
 *  - q (text)
...
export async function GET(req: Request, { params }: { params: { parish: string; town: string } }) {
  try {
    ...
    let items = getTransportForParish(parish);

    if (typeFilter) items = items.filter(i => (i.type || "").toLowerCase() === typeFilter);
    if (q) items = items.filter(i => ((i.name || "") + " " + (i.operator || "") + " " + (i.notes || "")).toLowerCase().includes(q));

    const total = items.length;
    const start = (page - 1) * limit;
    const paged = items.slice(start, start + limit);
    const meta = buildMetaForTransport(paged);

    return ok({ parish, town: targetTown, page, limit, total, transport: paged, meta });
  } catch (e:any) {
