...
export default async function ParishPage({ params }: { params: { parish: string } }) {
  const parishName = params.parish.replace(/-/g, ' ');
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
          {towns.map((town) => (
            <Link
              ...
              className="bg-gradient-to-br from-yellow-600 to-green-600 p-10 rounded-3xl text-center font-bold text-2xl hover:scale-110 transition-all shadow-xl"
            >
              {town}
            </Link>
          ))}
